...
> **Name the agent at init.** The `--agent-caller` value is set **once, at account creation** and cannot yet be changed afterward from the CLI ([backend#1199](https://github.com/ditto-assistant/backend/issues/1199)). It labels every memory the agent saves: after the account is claimed, the agent's memories appear in the owner's graph as an *external agent thread* titled by this name. The example above uses `hermes`; if the user has already chosen a name for their agent, pass that instead (e.g. `--agent-caller "ada"`). Decide the name before running `init` — don't omit `--agent-caller`, or the name defaults to the generic `agent`.
