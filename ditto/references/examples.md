...
1. Create a temporary claimable agent account:
   ...
   ```
   This returns a working API key plus a short `claimURL` for later human ownership. Share the `claimURL`, not the `ditto_mcp_...` API key. The claim token is carried in the query string (`?t=...`). Do not ask the user for email, OTP, browser login, or dashboard setup.
